...
axiosMock.onGet(/\/v1.0\/iot-03\/devices\/mock-id\/status/).reply(200, {
  ...
  result: [
    ...
    },
    {
      code: "flash_scene_4",
      value:
        '{"bright":255,"frequency":80,"hsv":[{"h":0.0,"s":255.0,"v":255.0},{"h":120.0,"s":255.0,"v":255.0},{"h":240.0,"s":255.0,"v":255.0},{"h":300.0,"s":255.0,"v":255.0},{"h":240.0,"s":255.0,"v":255.0},{"h":0.0,"s":255.0,"v":255.0}],"temperature":255}',
    },
  ],
});
...
axiosMock
  .onPost(/\/v1.0\/iot-03\/devices\/mock-id\/actions\/reset/)
  .reply(200, {
    success: true,
    t: 1550642917632,
    result: true,
  ...
  result: {
    ...
    functions: [
      ...
      },
      {
        ...
        type: "Json",
        values:
          '{"h":{"min":1,"scale":0,"unit":"","max":360,"step":1},"s":{"min":1,"scale":0,"unit":"","max":255,"step":1},"v":{"min":1,"scale":0,"unit":"","max":255,"step":1}}',
      },
    ],
    status: [
      {
...
axiosMock
  .onPost(/\/v1.0\/iot-03\/device-registration\/tokens\/mock-token/)
  .reply(200, {
    result: {
      error_devices: [],
      success_devices: [
...
axiosMock
  .onPost(
    /\/v1.0\/iot-03\/device-registration\/devices\/mock-id\/actions\/discover/,
  )
  .reply(200, {
    result: {
      error_devices: [],
...
axiosMock
  .onPost(
    /\/v1.0\/iot-03\/device-registration\/devices\/mock-id\/actions\/stop-discover/,
  )
  .reply(200, {
    success: true,
    t: 1551863646940,
...
axiosMock
  .onPost(/\/v1.0\/iot-03\/device-registration\/devices\/mock-id\/sub-devices/)
  .reply(200, {
    result: [
      {
